...
def get_houseid(prjid):
    ...
    tree=etree.HTML(xm_res.text)
    
    dl_list= tree.xpath('/html/body/div[4]/div[4]/div[4]/dl')
    for i in range(1,len(dl_list)+1):
        href= tree.xpath('/html/body/div[4]/div[4]/div[4]/dl[i]/dd[1]/h4/a/@href/text()')[0]
        channel = tree.xpath('/html/body/div[4]/div[4]/div[4]/dl[i]/dd[1]/h4/a/@data_channel/text()')[0]
        psid = tree.xpath('/html/body/div[4]/div[4]/div[4]/dl[i]/dd[1]/h4/a/@ps/text()')[0]

        house_href= f'https://nanjing.esf.fang.com/{href}?channel={channel}&psid={psid}'
        print(house_href)


        info=(href,channel,psid)
        sql = f"""replace into ftx_house (href,channel, psid) values ({('%s,' * 3)[:-1]})"""
...
def main_hosue():
    ...
    df=pd.read_sql('select prjid from project',conn)
    print(df.shape[0])
    for prjid in df['prjid']:
        try:
            get_houseid(prjid)
